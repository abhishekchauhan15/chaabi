...
const Words = ({
    ...
    if (selector !== index) {
      if (selector > index) {
        if (wordCorrect[index] === "false")
          return (
            <span id={`wrong${index}`} key={index} className="wordIncorrect">
              {word + " "}
            </span>
          );
        ...
        return (
          <span key={index} id={`${index}`} s className="word">
            {word + " "}
          </span>
        );
      }
      ...
      return (
        <span
          ...
          id="wordSelected"
          key={index}
          style={{ background: `${hlBackground}` }}
          className="wordSelected"
        >
          {word + " "}
        </span>
      );
  ...
  return <div style={{ marginTop: `${topDisplacement}px` }}>{renderWords}</div>;
};
